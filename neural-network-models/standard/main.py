...
# Loading datasets - origDims are the original image dimensions, required for converting the predicted outputs from a
...
trainingData = model.fit(X_train, Y_train, epochs = 2000, batch_size = 32, validation_data = (X_valid, Y_valid))
...
output = np.reshape(output, origDims)
plt.imshow(output)
plt.axis('off')
# Exact directory might be required below
